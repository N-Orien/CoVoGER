...
train_data = torch.load(f'{data_dir}/train_{dataset2}_{srclang}_{tgtlang}_{task}_{seamless_size}.pt')#, map_location=torch.device('cpu'))
val_data = torch.load(f'{data_dir}/dev_{dataset2}_{srclang}_{tgtlang}_{task}_{seamless_size}.pt')#, map_location=torch.device('cpu'))
...
run_name = f'gentrans_{dataset1}_{dataset2}_{srclang}_{tgtlang}_{task}_{seamless_size}'  # added z at end to distinguish
...
#    , loggers=logger)
    ...
    fabric.launch(main, checkpoint_dir, adapter_path, out_dir)


def main(fabric: L.Fabric, checkpoint_dir: Path, adapter_path: str, out_dir: Path):
