...
from litgpt.prompts import has_prompt_style, load_prompt_style, PromptStyle, Alpaca
    ...
    *,
    ...
    with open(json_path, "r", encoding="utf-8") as f:
        ...
        for i, rec in enumerate(records):
            ...
            inp = rec["input"].strip()
            prompt = Alpaca().apply(prompt=instr, input=inp)
            prompt = prompt_style.apply(prompt)
            encoded = tokenizer.encode(prompt, device=fabric.device)
            max_prompt_length = max(encoded.size(0), max_prompt_length)
            encoded_list.append(encoded)
            if i > 1000:
                break
    max_returned_tokens = max_prompt_length + max_new_tokens
